...
import { getChainNameNFTGO, getSeaportAddressForChain } from "@/lib/helpers";
...
export default function OfflinePage() {
    ...
    const wasReciver = !!!searchParmas?.get("wasReciver");
    const oNfts = searchParmas?.get("Onfts");
    const cNfts = searchParmas?.get("Cnfts");
    ...
    const [youUser, setYouUser] = useState<IUser>(initialUser);
    ...
    useEffect(() => {
        if (!chainId || !address || !addy) return;

        fetchOtherPartyInventory();
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [chainId, address, addy])
                        ...
                        <Card className="max-h-[90vh] h-full">
                            <CardContent >
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem value="you" disabled={userLoading}>
                                            {userLoading ? (
                                                ...
                                                <div className="flex items-center space-x-4 h-8">
                                                    <Avatar variant='beam' name={youUser.address.toLowerCase()} className="h-6 w-6" />
                                                    <div className="flex-grow">
                                                        <p className="font-medium">{youUser?.name && "@"}{youUser?.name || youUser.address || addy}</p>
                                                        {/* <p className="text-sm text-muted-foreground">{meUser?.address}</p> */}
                                                    </div>
                                                </div>
                                            )}
                                ...
                                <NftGallery
                                    ...
                                    fillers={16}
                                    maxHeight="75vh"
                                    onPressed={(nft) => selectedNftInventory(nft as INFTGONFT, inventorySelected === "me" ? true : false)}
